...
def build_news_and_capability_items(conn: sqlite3.Connection, items: list[dict], *, run_id: str) -> dict[str, int]:
    ...
    for item in items:
        ...
        news_id = repo.create_domain_item(
            ...
            tags=["raw_pipeline", payload.get("source_type") or item.get("source_type")],
            metrics={"pipeline_run": run_id},
            payload=payload,
        )
        news_count += 1
        repo.create_evidence(
            ...
            content=payload.get("summary") or "待补充正文和模型审阅。",
            source_url=payload.get("url") or "",
            confidence=None,
            payload={"item_key": item.get("item_key"), "run_id": run_id},
        )
        if payload.get("source_type") == "repo" or payload.get("code_url"):
