...
NOISE_TERMS = ["survey", "benchmark", "leaderboard", "dataset"]
...
def classify_news_item(item: dict[str, Any]) -> ClassificationResult:
    ...
    elif source_type == "repo" or code_hits:
        category = "安全工具与代码"
        subcategory = "开源仓库"
    elif hits:
        category = "AI 安全研究"
    ...
    confidence = min(0.95, 0.35 + 0.12 * len(hits) + 0.1 * len(vuln_hits) + 0.08 * len(agent_hits) + 0.05 * len(code_hits) - 0.04 * len(noise_hits))
    ...
    if code_hits or source_type == "repo":
        reasons.append("包含代码或仓库线索")
    if not reasons:
        reasons.append("未命中强安全关键词，需要人工复核")
    return ClassificationResult(category=category, subcategory=subcategory, tags=tags, confidence=round(max(0.05, confidence), 2), reasons=reasons, signals={"source_type": source_type, "ai_security_hits": hits, "vuln_hits": vuln_hits, "agent_hits": agent_hits, "code_hits": code_hits, "noise_hits": noise_hits})
