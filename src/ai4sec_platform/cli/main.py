...
from ai4sec_platform.cli import audit_run, database, init_db, inspect_run, pipeline_worker, run_pipeline


def main() -> int:
    parser = argparse.ArgumentParser(description="AI4SEC Platform CLI")
    parser.add_argument("command", choices=["init-db", "run-pipeline", "inspect-run", "audit-run", "database", "pipeline-worker"])
