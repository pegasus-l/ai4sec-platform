...
class XFeedConnector(NewsLiveConnector):
    ...
    def fetch(self, request: SourceFetchRequest) -> SourceFetchResult:
        ...
        for account in accounts:
            ...
            try:
                raw = json.loads(self.get_bytes(url, headers={"Authorization": f"Bearer {api_key}"}).decode("utf-8"))
                ...
                for tweet in tweets:
                    ...
                    items.append({"id": f"x:{tweet.get('id')}", "title": str(tweet.get("text") or "")[:120], "text": tweet.get("text", ""), "url": tweet.get("url", ""), "reference_urls": [value for value in urls if value], "author": author.get("userName", username), "published": tweet.get("createdAt", ""), "source": "x"})
