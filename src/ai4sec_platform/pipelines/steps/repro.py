...
def _build_repro_prompt(code_url: str, task_id: int) -> str:
    ...
    return (
        ...
        f"/root/.cache/opencode/models.json 里, endpoint=https://dashscope.aliyuncs.com/compatible-mode/v1, 可用模型 glm-5.1。"
        ...
        f"   a. 部署必须 Docker: 唯一/必需部署方式是 docker run 或 docker-compose, 没有可行的非 Docker 原生运行路径。\n"
        f"   b. 核心功能必须 Docker: 项目最核心的用户价值闭环/关键运行阶段【只能】在 Docker 容器/沙箱里跑"
        f"(例如\"漏洞验证靠隔离容器沙箱执行\"、\"执行不可信代码靠容器隔离\"、\"只能以容器镜像方式运行\"), 没有 Docker 时核心链只能降级/mock/根本跑不通。\n"
        f"只要 a 或 b 成立, 立即停止: 本环境不支持 Docker, 按平台规则此类项目【不复现】。如实报告 is_web=false、web_started=false, "
        f"blockers 写\"项目必须 Docker(部署或核心功能), 本复现环境不支持 Docker, 按平台规则不复现\", FINAL_VERDICT: FAILURE, 然后结束。"
        f"不要尝试安装/模拟 Docker, 不要浪费时间折腾, 不要凑合出一个 partial。\n"
        f"Docker 只是【可选/辅助】能力(存在可用的原生部署路径, 且核心用户闭环不依赖 Docker 也能跑通)时, 不触发本规则, 照常按下面流程原生部署, "
        f"并把因无 Docker 而缺失的能力写进 limitations。\n"
        ...
        f"- Python 项目依赖必须装进项目自己的 venv(`python3 -m venv .venv && .venv/bin/pip install -r requirements.txt`); "
        f"【不要】pip install 到系统级(容器重建即丢); 启动命令一律用 venv 内绝对路径(`.venv/bin/streamlit` / `.venv/bin/uvicorn`), 不要用裸命令名。\n"
        ...
        f"启动命令必须用 venv 内绝对路径(如 .venv/bin/streamlit / .venv/bin/uvicorn), 不要用裸命令名(系统级依赖容器重建会丢, 裸命令起不来); "
        f"脚本开头加幂等引导守卫: 入口缺失时先在 flock 锁内建 venv 装依赖再启动, 避免 watchdog 每 10s 重试并发重复建 venv, 写法参考 /workspace/.repro-web/task-15.sh;\n"
...
_SESSION_RE = re.compile(r"已创建复现会话 (\S+)")


def _session_id_from_log(log: str) -> str:
    m = _SESSION_RE.search(log or "")
...
def recover_orphaned_tasks() -> int:
    ...
    try:
        rows = conn.execute(
            "SELECT id, item_id, status, log FROM capability_repro_tasks "
