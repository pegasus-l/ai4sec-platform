...
def _build_repro_prompt(code_url: str, task_id: int) -> str:
    ...
    return (
        ...
        f"环境注意: 本环境没有 Docker、没有 GPU(部署必须 Docker 的项目请看第零步①, 直接不复现)。"
        ...
        f"# 第零步(最重要): ① 先判断部署是否必须 Docker(必须则不复现) → ② 再判断项目【本身】有没有 Web 界面\n"
        f"① 读 README/部署文档/看仓库里有没有 docker-compose.yml、Dockerfile: 若项目【部署必须依赖 Docker】"
        f"(唯一/必需部署方式是 docker run 或 docker-compose, 没有可行的非 Docker 直接运行路径), 立即停止: 本环境不支持 Docker, "
        f"按平台规则此类项目【不复现】。如实报告 is_web=false、web_started=false, blockers 写\"部署必须 Docker, 本复现环境不支持 Docker, 按平台规则不复现\", "
        f"FINAL_VERDICT: FAILURE, 然后结束。不要尝试安装/模拟 Docker, 不要浪费时间折腾。"
        f"Docker 只是【可选】部署方式之一(还存在可用的原生部署路径)时, 照常按下面流程原生部署。\n"
