...
- [x] 连续运行至少三个真实日更周期；`2026-08-05`、`2026-08-06`、`2026-08-07` 三个不同业务日期均通过真实 Provider、启用源、Schema、人工队列和日报验收，同日再次触发会复用成功 Run，不重复采集、模型调用或生成日报。
...
### 2026-08-03：完成正式能力洞察前端页面

- 没有新建第二套“生产版”页面，而是在现有能力洞察工作台上完成收口，避免正式页面与 Beta/demo 页面长期并存。
- `/api/capabilities/repro-runs` 删除旧 demo 聚合字段和 `repro-{item_id}` 字符串 ID，改为与单任务详情相同的 `ReproTaskResponse` 正式契约。任务列表、详情、停止和清理操作现在统一使用真实数值任务 ID，并稳定返回 `item_id`、策略、端口、状态和结构化报告。
- 前端删除 `task_id/display_id` 兼容字段，显示 local_web/CLI 策略、报告 Schema 版本、真实运行命令、验收问题、证据和限制；非 Web 复现的验收失败原因不再被隐藏在 Web 专属区域。
- 成功和部分成功指标分开统计，避免把 `partial` 包装成成功；搜索框改为真实受控筛选，刷新操作改为覆盖全部 `cap-*` 查询，接口失败时显示错误状态而不是伪装成空数据。
- 新增正式任务列表契约回归，验证列表 ID、能力条目 ID 和复现策略与数据库任务一致；能力相关定向测试 37 passed，前端 TypeScript 与 Vite 生产构建通过。构建仍有既有的动态/静态 import 和大 chunk 警告，后续作为平台前端性能优化处理，不阻塞本项业务验收。

### 2026-08-03：补齐能力复现 API、SSE 与回写回归

- 现有测试已覆盖入队不执行、持久 stop/cleanup 请求、独立 Worker 清理、资源边界、SSE 终态关闭、配额、Worker 健康、策略审批和状态机；本批次补齐验收缺口，而不是重复建设第二套测试框架。
...
### 2026-08-06：资讯三周期真实日更与触发级幂等验收通过

- 在隔离数据库 `output/repro-regression/ai4sec_platform.db` 完成 `2026-08-05`、`2026-08-06`、`2026-08-07` 三个不同业务日期的真实 `news.daily_pipeline`。最终 Run 分别为 `run_20260805111945_ef05fa04f5`、`run_20260805114031_9f3cd8fbbf`、`run_20260805115951_fe198f7f7d`，`news-acceptance` 返回 `status=pass`、`qualified_cycles=3`、`remaining_cycles=0`；模型 Provider 为 DashScope `glm-5.2`，不是 `local_rules`。
- 三周期原始采集量分别为 2099、1431、1424，去重后候选分别为 2100、1854、1847，最终入选分别为 14、17、22；最终成功 lineage 的门控与深评 Schema 通过率均为 100%，三份日报质量审计分数均为 1.0。arXiv、GitHub、WeRSS、ASIS、Awesome 均有健康记录且无最终采集错误；X 按已批准决策保持正式禁用并展示原因，不以伪健康状态计入验收。
- WeRSS 与 ASIS 在首周期分别采集 300、100 条，后续两个周期均为 0，证明已扫描 ID 水位线生效，而不是每个日更重复读取全部历史记录。第二周期有一个模型连续返回非法 `tech_paths` 的候选，经正式人工队列拒绝后恢复成功；其他 Schema 异常通过失败项缓存与 checkpoint 重试恢复，失败历史仍保留审计。
...
- 第二批 OpenFuyao、OpenGauss 真实 full 采集 234 个项目、353 条组织安全材料，耗时约 35 秒；CVE scout 产出 451 个唯一 CVE。按组织对照旧数据：OpenFuyao 当前 118 项目/4 个带安全数据/47 个唯一 CVE，旧为 107/1/0；OpenGauss 当前 116/17/405，旧为 117/24/273。OpenFuyao 的新增 CVE 需要后续人工抽样确认是否为当前 security repo 的真实新增，而不是关键词误收。
- 第三批 Cangjie、Cantian、ModelEngine、OpenHiTLS 采集 70 项目并完成 CVE scout；Cangjie 当前 1 个唯一 CVE 与旧一致，Cantian/ModelEngine/OpenHiTLS 均无新增唯一 CVE，主要差异是 issue 深分页带来的 broad security 增长。
- 第四至第六批完成剩余组织：Ascend、CANN、MindSpore、OpenUBMC、HuaweiCloud、OpenHarmony SIG/TPC、OpenKylin、DevCloudFE、arkui-x、eBackup、kappital、KunpengCompute、openInula、openJiuwen、OpenLookeng、OpenTiny。OpenKylin 使用 60 页后得到 4375 项目，与旧基线完全一致；25 个组织最终全部覆盖。
- 合并所有分批 artifact 后，当前真实 full 结果为 7946 项目、437 个带安全数据项目、4505 条 CVE 引用、3967 个唯一 CVE、8875 条 broad security；旧结果为 8065、311、1429、777、1203。项目数差异主要来自 OpenHarmony SIG `441 vs 699` 和若干组织仓库迁移/删除；CVE 与 broad security 大幅增长主要来自当前 issue/security repo 深分页，必须经过 shadow sample 人工校准后才能调整质量阈值或宣布覆盖提升。OpenKylin security repo 识别但安全材料为空，质量审计已正确告警。
