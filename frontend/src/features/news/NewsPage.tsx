...
function NewsCard({ item, onSelect }: { item: NewsItem; onSelect: (item: NewsItem) => void }) {
  return <button className="news-card" onClick={() => onSelect(item)}><div className="news-card-top"><Badge tone={item.item_type === 'paper' ? 'green' : 'violet'}>{typeLabel(item.item_type)}</Badge><Badge tone="sky">{sourceLabel(item.source)}</Badge><span>{displayTopic(item)}</span></div><h3>{displayTheme(item)}</h3>{displayTheme(item) !== item.title && <div className="original-title">{item.title}</div>}<div className="content-line"><b>宣传</b><span>{promoLine(item)}</span></div><div className="content-line highlight-line"><b>亮点</b><span>{highlightLine(item)}</span></div><div className="tag-line">{item.technical_points.map(point => <span key={point}>{point}</span>)}</div><div className="news-card-bottom"><span>{formatDate(item.primary_date)}</span><strong>{Math.round(item.score || 0)}</strong></div></button>;
}
...
function workName(item: NewsItem): string { return String(item.payload.display_work_name || ''); }
function themeDescriptor(item: NewsItem): string { return String(item.payload.display_theme_descriptor || ''); }
