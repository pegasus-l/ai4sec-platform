...
import { fetchNewsOps, fetchNewsRun, retryNewsRun, retryNewsSource, startNewsPipeline } from './newsQueries';
...
type RunDetail = Run & { tasks: Task[]; artifacts: Artifact[]; models: ModelMetrics; retry: { allowed: boolean; stage: string; mode: string } };
...
type Source = { id: string; name: string; status: string; health: string; latest_at: string; item_count: number; collected_count: number; error_count: number; errors: string[]; disabled: boolean; disabled_reason: string; consecutive_failures: number; last_success_at: string; last_health_check: { checked_at?: string; latency_ms?: number; message?: string }; summary: Record<string, unknown> };
...
function Overview({ setView }: { setView: (view: NewsView) => void }) {
  ...
  return <div className="news-ops-stack">
    ...
    <div className="news-ops-grid"><Card><div className="ops-card-title"><Server size={17} /><h3>六数据源</h3></div><div className="ops-source-strip">{data.sources.map(source => <button key={source.id} onClick={() => setView('ops-sources')}><Status value={source.disabled ? 'disabled' : source.error_count ? 'degraded' : source.health || source.status} /><b>{sourceNames[source.id]}</b><span>{source.disabled ? source.disabled_reason : `采集 ${source.collected_count} · 入库 ${source.item_count}${source.error_count ? ` · 失败 ${source.error_count}` : ''}`}</span></button>)}</div></Card><Card><div className="ops-card-title"><FileText size={17} /><h3>日报产物</h3></div>{data.latest_report ? <><strong className="ops-run-name">{data.latest_report.title}</strong><p>最新日报：{data.latest_report.report_date}</p></> : <p className="muted">尚未生成日报，完成 Pipeline 后自动生成。</p>}</Card></div>
  </div>;
}
...
function RunDrawer({ data, open, close }: { data?: RunDetail; open: boolean; close: () => void }) {
  const queryClient = useQueryClient();
  const retry = useMutation({ mutationFn: (runId: string) => retryNewsRun(runId), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['news-ops'] }) });
  return <Drawer open={open} title={data?.pipeline_name || '正在加载运行详情'} subtitle={data?.run_id} onClose={close}>{data && <div className="news-detail"><Card><div className="ops-card-title"><Status value={data.status} /><span>{formatTime(data.started_at)} → {formatTime(data.finished_at)}</span></div><p>Shadow 模式：{data.production_writes ? '否' : '是'}</p>{data.retry.allowed && <div className="ops-actions"><button className="primary" disabled={retry.isPending} onClick={() => retry.mutate(data.run_id)}><RefreshCw size={14} />{retry.isPending ? '正在创建恢复任务' : `从失败阶段重跑：${stepNames[data.retry.stage] || data.retry.stage}`}</button></div>}</Card><div className="ops-steps">{data.tasks.map((task, index) => <Card key={task.id}><div className="ops-step-index">{index + 1}</div><div><div className="ops-card-title"><strong>{stepNames[task.step_name] || task.step_name}</strong><Status value={task.status} /></div><p>{compactMetrics(task.metrics)}</p>{task.error_message && <div className="ops-error">{task.error_message}</div>}</div></Card>)}</div><Card><h3>模型调用</h3><p>共 {data.models.total} 次 · 成功 {data.models.success} · 失败 {data.models.failed} · 重试 {data.models.retryable_failure}</p></Card><Card><h3>原始与中间产物 · {data.artifacts.length}</h3>{data.artifacts.map(item => <div className="ops-artifact" key={item.id}><Database size={14} /><span>{item.artifact_type}</span><code>{item.path}</code></div>)}</Card></div>}</Drawer>;
}

function Sources() {
  const queryClient = useQueryClient();
  const query = useQuery({ queryKey: ['news-ops', 'sources'], queryFn: () => fetchNewsOps<{ items: Source[] }>('sources'), refetchInterval: 10000 });
  const retry = useMutation({ mutationFn: (source: string) => retryNewsSource(source), onSuccess: () => queryClient.invalidateQueries({ queryKey: ['news-ops'] }) });
  return <div className="ops-source-grid">{query.data?.items.map(source => <Card key={source.id}><div className="ops-card-title"><Server size={17} /><h3>{sourceNames[source.id]}</h3><Status value={source.disabled ? 'disabled' : source.error_count ? 'degraded' : source.health || source.status} /></div><strong className="ops-source-count">{source.collected_count}</strong><span className="muted">本次获取 · 入库 {source.item_count}</span><dl><dt>最近更新</dt><dd>{formatTime(source.latest_at)}</dd><dt>最近健康探测</dt><dd>{formatTime(source.last_health_check.checked_at)}</dd><dt>最近成功</dt><dd>{formatTime(source.last_success_at)}</dd><dt>连续失败</dt><dd>{source.consecutive_failures}</dd><dt>探测延迟</dt><dd>{source.last_health_check.latency_ms === undefined ? '—' : `${source.last_health_check.latency_ms}ms`}</dd><dt>采集摘要</dt><dd>{compactMetrics(source.summary)}</dd></dl>{source.disabled && <div className="ops-source-errors"><strong>禁用原因</strong><p>{source.disabled_reason}</p></div>}{source.error_count > 0 && <div className="ops-source-errors"><strong>失败原因</strong>{source.errors.map((error, index) => <p key={`${source.id}-${index}`}>{error}</p>)}</div>}<div className="ops-actions"><button className={source.error_count ? 'primary' : ''} disabled={retry.isPending || source.disabled} onClick={() => retry.mutate(source.id)}><Play size={14} />{source.disabled ? '数据源已禁用' : retry.isPending ? '正在启动' : source.error_count ? '重跑失败来源' : '重新采集该来源'}</button></div></Card>)}</div>;
}
...
function Status({ value }: { value: string }) { const good = ['ok', 'success', 'healthy', 'pass', 'completed'].includes(value); const bad = ['failed', 'error', 'unhealthy', 'fail', 'auth_failed', 'quota_exhausted', 'rate_limited', 'upstream_failed', 'timeout'].includes(value); return <Badge tone={good ? 'green' : bad ? 'red' : value === 'running' ? 'sky' : 'amber'}>{value === 'success' ? '成功' : value === 'running' ? '运行中' : value === 'failed' ? '失败' : value === 'disabled' ? '已禁用' : ['ok', 'healthy'].includes(value) ? '健康' : value === 'unknown' ? '未知' : value}</Badge>; }
