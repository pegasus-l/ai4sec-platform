...
function Status({ value }: { value: string }) { const good = ['ok', 'success', 'healthy', 'pass', 'completed'].includes(value); const bad = ['failed', 'error', 'unhealthy', 'fail'].includes(value); return <Badge tone={good ? 'green' : bad ? 'red' : value === 'running' ? 'sky' : 'amber'}>{value === 'success' ? '成功' : value === 'running' ? '运行中' : value === 'failed' ? '失败' : value === 'disabled' ? '已禁用' : ['ok', 'healthy'].includes(value) ? '健康' : value === 'unknown' ? '未知' : value}</Badge>; }
