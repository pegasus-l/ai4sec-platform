...
CVE 权威组件校验默认关闭，避免每次扫描额外请求外部服务。可通过 `cve_authority_mode=cache` 只读本地缓存，或通过 `cve_authority_mode=live` 在缓存缺失时查询 CVE.org；默认只校验项目扇出大于等于 5 的前 25 个 CVE，可用 `cve_authority_min_fanout` 和 `cve_authority_limit` 调整。结构化 CVE ID 与完整来源 ID 冲突可在本地直接识别；组件冲突时，如果权威产品同时明确出现在漏洞简述中，则自动确认为来源组件字段误标，否则进入人工复核。两类记录都保留证据并排除项目自身风险；查询失败、缓存缺失或损坏不会自动判为误收。
