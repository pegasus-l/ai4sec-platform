...
from ai4sec_platform.domains.capabilities.model_gateway import authorize_task_model_call, issue_task_model_token, revoke_task_model_tokens
...
def test_task_model_token_enforces_model_calls_tokens_and_revocation() -> None:
    ...
    assert authorize_task_model_call(conn, token=token, model="glm-5.2", requested_tokens=500) is None
    assert authorize_task_model_call(conn, token=token, model="glm-5.2", requested_tokens=300)
